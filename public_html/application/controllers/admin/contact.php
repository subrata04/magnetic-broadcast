        ...
        */
        ...
        if (count($_POST) > 0) {
           ...
           if ($this->form_validation->run() !== FALSE) {    // If data is valid then insert into database
                ...
                $s_where = "(`s_fname` LIKE '%" . $s_search_val . "%' OR `s_lname` LIKE '%" . $s_search_val . "%' OR `s_cname` LIKE '%" . $s_search_val . "%' OR `s_email` LIKE '%" . $s_search_val . "%')";
            }
        }

        // Multiple data fetching [start]
        $s_tab_name = 'contact_details';
        $s_select = "*";
        //pr(get_ses_data('i_roles'),true);
            ...
            $s_order_by_name = 'id';
            $s_order_by = 'DESC';
            $m_user_dataset = $this->mod->fetchMultiData($s_tab_name, $s_select,$s_where, $s_group_by, $s_order_by_name, $s_order_by,$this->item_per_page,intval(($i_page-1)*$this->item_per_page));
           // echo $this->db->last_query();
            $i_total_rows = $this->mod->fetchMultiDataCount($s_tab_name, $s_where, $s_group_by);
            if($i_total_rows>0){
                ...
                if($i_last_page<$i_page){   // If no data found            
                    redirect(admin_url().'contact/listing/'.$i_last_page);
                }
            }
        ...
        $m_conf['base_url'] = admin_url().'contact/listing/';
...
echo 1;
    ...
    public function ban_user(){
        if(count($_POST)>0){
            ...
            if($ret_){
                add_msg("User blocked successfully.", 'ok');
    ...
    */
    ...
    public function change_state($s_state='', $s_enc_id='',$s_back_url=""){
        ...
        if($ret_){
            ($i_is_allow==1)?add_msg("User unblocked successfully.", 'ok'):add_msg("User blocked successfully.", 'ok');
        }else{
            add_msg("User status change not done");
        }
        ...
        switch($s_action){
            ...
            case 1: // CASE DELETE
                foreach($m_ids as $s_id){
                    ...
                    $dataset = $this->mod->fetchSingleData('contact_details',$m_where);

                    $ret_ = $this->mod->delData('contact_details',$m_where);
    ...
    */
    public function edit($s_edit_id='',$s_back_url='') {
        ...
        $s_back_url=(!empty($s_back_url))?strDecode($s_back_url):admin_url()."user/listing.html";
        $i_edit_id = intval(strDecode($s_edit_id));
    ...
    */
    public function del($s_id='',$s_back_url='') {
      ...
      //  $this->check_user_access('delt_user', get_ses_data('i_roles'));
        ...
        // Message setting
        if ($ret_ > 0) {
            add_msg('Contact user deleted successfully!!', "ok");
        } else {
            add_msg('Contact user not deleted!!', "err");
        }
        redirect($s_back_url);
    }
    ...
    */
    public function user_info_ajax($s_id="") {
        ...
        if ($i_id == 0) {
            ...
            $this->load->view(ADMIN_FOLDER . "/user/user_info_ajax.tpl.php", $m_data);
        }
    }
    ...
    */
    public function personal_info_ajax($s_id="") {
        ...
        if ($i_id == 0) {
            ...
            $this->load->view(ADMIN_FOLDER . "/user/personal_info_ajax.tpl.php", $m_data);
        }
    }
    ...
    */
    public function user_accn_ajax($s_id="") {
        ...
        if ($i_id == 0) {
            ...
            $this->load->view(ADMIN_FOLDER . "/user/user_accn_ajax.tpl.php", $m_data);
        }
    }
    /**
    * function for showing popup for user info account
    ...
    */
    public function my_accn_ajax($s_id="") {
        ...
        if ($i_id == 0) {
            ...
            // data setting to array
            $m_data['m_send_data'] = $retset;
            $this->load->view(ADMIN_FOLDER . "/user/my_accn_ajax.tpl.php", $m_data);
        }
    }
